    ...
    for s, side, krw in orders:
        ...
        try:
            if side == "buy" and not buys_started:
                # 결함 E-KR: 매도 접수 후 '주문가능현금'(당일 매도대금 재사용 포함)을 조회해 95%만 예산으로 사용
                try:
                    time.sleep(1.5)  # 매도 체결 반영 대기
                    bp = get_buying_power()
                    live_cash = bp["ord_psbl_cash"]
                    lines.append(f"- 매수가능조회: 주문가능현금 {bp['ord_psbl_cash']:,.0f} / 재사용가능 {bp['ruse_psbl_amt']:,.0f}"
                ...
                lines.append(f"- 매수 예산: 주문가능현금 {live_cash:,.0f}원 × {CASH_BUFFER:.0%} = {budget:,.0f}원")
                buys_started = True
            px = get_price(s)
