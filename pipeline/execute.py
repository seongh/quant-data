...
from preflight import HOLIDAYS  # 휴장일 단일 소스 (preflight와 동일 캘린더 — 결함 F 패치, 2026-09-08)
...
def market_open_now(now=None) -> tuple[bool, str]:
    """뉴욕 정규장(평일 09:30~15:45 ET, 휴장일 제외) 여부. 장외 발화(사고 #7 지연)는 주문 없이 종료."""
    now = now or datetime.now(NY)
    hm = now.hour * 60 + now.minute
    if now.weekday() >= 5:
        return False, f"주말 (NY {now:%Y-%m-%d %H:%M})"
    if f"{now:%Y-%m-%d}" in HOLIDAYS["us"]:  # 결함 F 패치: 휴장일 발화 → 큐잉 방지 (9/7 Labor Day 실증)
        return False, f"휴장일 (NY {now:%Y-%m-%d %H:%M})"
