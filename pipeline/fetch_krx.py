...
def stored_universe() -> dict[str, str]:
    """저장된 최신 parquet의 (ticker, name) — 외부 리스팅 실패 시 폴백 (X5, 2026-09-11).
    종목 구성은 하루 단위로 바뀌지 않으므로 증분 수집에는 저장본 목록으로 충분하다."""
    files = sorted(KRX_DIR.glob("*.parquet"))
    if not files:
        return {}
    df = pd.read_parquet(files[-1], columns=["ticker", "name"]).drop_duplicates("ticker")
    return {str(t).zfill(6): str(n) for t, n in zip(df["ticker"], df["name"])}


def get_universe() -> dict[str, str]:
    """KOSPI 시총 상위 TOP_N + ETF. 리스팅 소스는 순차 폴백 → 최종 폴백은 저장본 목록."""
    listing = None
    ...
    if listing is None:
        uni = stored_universe()
        if len(uni) < 100:
            raise RuntimeError("종목 리스팅 소스 전부 실패 + 저장본 목록도 없음")
        print(f"[warn] 외부 리스팅 전부 실패 → 저장본 목록 {len(uni)}개로 증분 수집 (생존편향 문서화 한계 동일)",
              file=sys.stderr)
        uni.update(ETFS)
        return uni
