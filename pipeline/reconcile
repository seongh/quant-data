...
def main():
    ...
    if mkt == "us":
        ...
        if key and sec:
            ...
            if cash < 0.0:  # 허용오차 제거 (게이트 B1: -$2도 1조 위반, 2026-09-08)
    ...
    if mkt == "kr":
        # 결함 H (2026-09-12): KR 계좌에는 현금 음수 검사가 없어 -35,969원 상태가 9/7~9/11 미검출. US와 동일 검사 추가.
        if os.environ.get("KIS_APP_KEY") and os.environ.get("KIS_APP_SECRET") and os.environ.get("KIS_ACCOUNT"):
            sys.path.insert(0, str(ROOT / "pipeline"))
            import execute_kr as kr
            time.sleep(30)  # 시장가 체결 반영 대기
            kr.get_token()
            _, equity, cash = kr.get_balance()
            print(f"[reconcile] KR 계좌 평가 {equity:,.0f}원 / 예수금 {cash:,.0f}원")
            if cash < 0.0:
                problems.append(f"KR 예수금 음수 {cash:,.0f}원 — 헌법 1조(무레버리지) 위반 상태. "
                                f"수수료·세금 미반영 매수가 원인이면 다음 집행에서 자동 축소되나 위원회 사고 등재 필요")
        else:
            print("[reconcile] KIS 키 없음 — KR 현금 검사 생략")
